...
## Your Turn #1 {background-color="#e3faf1" .smaller}
...
::: notes
To give you an example: the logical test here is whether or not the `mrn` value is equal to 
5 00 00 83. This is **false** for the first three rows.  In these rows, the `mrn` value is something else.  For the 4th row, however, it is **true** that the `mrn` value is equal to 5 00 00 83.
...
The logical operator for NOT equals is exclamation point equals. When we go through the covid_testing data frame row by row, we see this expression will be false for the first two rows where the last_name is “stark” and true for the last two rows where the last_name isn’t “stark”. 
The second point is that when you’re doing a comparison with a literal character string, such as “stark”, that needs to go into quotes.
:::
...
A is wrong because “positive” is a character string (it’s not a number or a logical value such as TRUE/FALSE).
B is wrong because you’re trying to do a comparison with a single equals.
C is correct!
D is wrong because it flips the positions of the comparison; the column name goes to the left and the comparator on the right.
...
1) The RStudio Workbench product Posit supports still uses R version 3.6 as its default, and we're going to use that.  Since that's older than version 4.1.0, we don't have the new pipe available to us and
2) We think the older pipe is still going to be what you see most.
...
## Your Turn #4 {background-color="#e3faf1" .smaller}
...
## Your Turn #5 {background-color="#e3faf1" .smaller}
...
::: notes
Now let's do some hands-on work.  Please make sure you're logged in to the RStudio server at rstudio.choprusergroup.org, then go to your exercises folder and open the 03 transform file.  You'll have five minutes to go through the instructions in that file!
...
`filter()`, which subsets rows by a logical condition , and
`mutate()`, which creates new calculated columns or changes existing columns
...
Depending on where in CHOP you work, you might have access to resources that CHQA offers, like Marx or CHOP's RStudio Connect.  If you work in Research at CHOP or if you work outside of CHOP, you may want to download R and RStudio to your own computer.  If it's a work computer, you might have to get IS to install this software for you.
...
:::
::: {.column width="80%"}
<https://bit.ly/chopRusers>
:::
:::

::: notes

The next thing that we recommend is to become part of a community of practice. The best way to do that is to join the CHOP R User group (which is also open to Penn folks). I can’t overstate the importance of having a place to go to ask for help when you start coding. The CHOP R User group has more than 400 members from all departments. 

This is great place to network with other Rusers, get help with coding problems, and learn about new packages, webinars, and conferences. If you are CHOP or Penn affiliated, please go to the bit.ly address now and sign up to be part of our mailing list and Slack!

We also have semi-regular meetings, and are always looking for new people to present short “lightning talks” about something they’ve been working on, a cool new package they’ve discovered, or really anything related to their experience with R. 

We welcome talks from a variety of experience levels, so as you continue to get your feet wet with R, consider whether there’s something you’d be open to presenting about! Either way, we hope you join us for our next meeting, which will be shared via the CHOPR email distribution list as well as in our Slack.
...
::: notes
Finally, I wanted to point out that “R For Data Science” by Garrett Grolemund and Hadley Wickham is the de-facto standard introduction to data science with R.
...
## Participate in / Improve This Workshop

![](media/chopr_logo.png){width="30%" fig-align="center"}

::: notes
This work belongs to the CHOP R User Group, and for those of you who are CHOP or Penn affiliated, we hope you'll consider helping us.  We need additional instructors, TAs, Zoomologists, and other helpers to be able to keep doing this workshop.

We'd like to be able to do more advanced workshops, too, but until we get additional helpers, we can't move forward with that.  If you're interested in participating, please reach out to either of the instructors or to Joy, and we'll coordinate with you.
...
Please, please, fill out the final survey we send out, it's a concrete way to show your gratitude!

::: notes
At this point, I would like to thank all the helpers: instructors, TAs, Zoomologists.

I'd also like to thank you for your participation!

** So to give them, and you, a round of virtual applause please click the “yes” button or another reaction button, like the party hat or the clapping hands, one last time!
