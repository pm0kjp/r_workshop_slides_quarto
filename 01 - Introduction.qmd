...
::: {.column width="33%" .small-text}
![](media/rstudio_logo.png){.two-thirds}
...
::: {.column width="33%" .small-text}
![](media/r_markdown_logo.png){.two-thirds}
...
Finally there's **R Markdown**. R Markdown is a **computational document**, an example of [**literate programming**](https://education.arcus.chop.edu/literate-statistical-programming/). A computational document is a document that has executable code inside of it, and as we'll discuss shortly, it's a great way to perform data analysis in a reproducible fashion. You'll also hear this called "literate programming" or "literate statistical programming", because an R Markdown document contains both human language (it's literate) as well as computer code (programming).  

With R Markdown, we will usually write that executable code using the R language. However, it's actually possible to write code in Python, C++, SQL, and other languages inside of an R Markdown document.  That goes beyond the scope of this workshop, but it's helpful to remember that R Markdown is quite flexible.

So we have R, Rstudio, and R Markdown. For the remainder of this session, I’ll show you how these three R’s come together to form the infrastructure for performing state of the art data analysis.
...
As I just mentioned, there are two versions of RStudio:

**RStudio Server** – a version of the RStudio IDE that can be accessed from a web browser. It's hosted on a server that could be on premises or in the cloud. We'll offer a version of RStudio in the cloud for your use today.  We ask you to use this server, even if you have R installed locally, because we've preloaded some files there for you to use.  
...
There are some small server issues that won't affect our work today but will create a couple of scary looking messages the first time you use `library`.
...
So, Joy asked me to apologize to you all on her behalf.  With the latest version of RStudio Workbench, there's a small error she's trying to chase down related to synching up the server clock.

You'll see these error messages, probably just one time, and you shouldn't see these if you work in R on your own computer in the future.  It's a weird artifact of how our server is set up.  While these messages may look scary, they won't affect our work today.
:::
...
Now it’s your turn to get logged into the RStudio training environment. 

The link to the training environment is in this morning's email, but I'm also going to ask Joy to paste the link into the Zoom chat. When you click on this link, it’ll ask you for a username and password. You can find your username and password in an Excel spreadsheet that’s attached to this morning’s reminder email. 

Please click the “yes” button in the Participants window to let us know you logged in successfully and can see the Rstudio panes. If you have any issues, please let us know in the chat!
...
![](media/excel_logo.png){fig-align="center" .one-third}
...
![](media/r_markdown_logo){.one-third fig-align="center"}

::: notes
Now that I have convinced you (I hope) that reproducibility is important, how do you undertake a reproducible analysis using the R language? One answer to this is **R Markdown**.
...
R Markdown is quickly becoming the standard for reproducible data analysis. A primary aim of this workshop is to teach you to use R Markdown and to encourage you to start working it into your day to day data workflows.

:::

## Anatomy of an R Markdown document

![](media/r_markdown_anatomy.png){.two-thirds fig-align="center"}
...
![](media/run_chunk.png){.two-thirds fig-align="center"}
...
::: {.columns}
::: {.column width="50%" .small-text}
![](media/knit_button.png){.bordered}

:::
::: {.column width="50%" .small-text}
![](media/rendered_markdown.png){.bordered}
...
If you are working in some forms of R Markdown, like R Notebooks, you'll see the word "Preview" instead of "Knit".  That also acts to render the Markdown.
:::
...
::: {.columns}
::: {.column width="50%" .small-text}
![](media/r_markdown_detail.png){.bordered}

:::
::: {.column width="50%" .small-text}
![](media/rendered_detail.png){.bordered}

:::
:::
::: notes
Let's look at this R Markdown document in a little more detail.  On the slide you see an R Markdown document and the resulting output in HTML that was created when this R Markdown document was knitted.

I'd like to explain what each part of the R Markdown includes in a bit more detail.
...
## Your Turn #2 {background-color="#e3faf1" .smaller}
...
Run each chunk by clicking the green "play" button (![](media/play.png){height="1em" .inline-image}).  Note what happens.

Knit the document with the "ball of yarn" button (![](media/knit.png){height="1em" .inline-image}). Type "test" and click Save to save the HTML file. Inspect the HTML document.
...
**Live Coding Section** (Switch to the RStudio server and do the exercise)
...
Can anyone tell me why is the gray code box missing in the rendered document? That's right, there's an echo=FALSE, which means "hey, please don't show this code in the final document."  "Echo" here means "print what we typed".  Sometimes showing your code is helpful, but sometimes it can be distracting, especially if you're using R Markdown to create a report that just shows results, for example, to show your PI some information about recruitment goals.  If your stakeholders don't want to see the code, but just the result of the code, use "echo = FALSE".
...
![](media/pipeline_import.png){.two-thirds fig-align="center"}
...
## Your Turn #3 {background-color="#e3faf1" .smaller}
...
Click (![](media/yes_button.png){height="1em" .inline-image}) when you are done.
...
Once again, I want you to go into the RStudio server, and this time you'll work with an existing file we've placed in the server for your use.  You'll go into the folder called "exercises" and fine the first assignment, called "Introduction".  Go ahead and open that, and read the rest of the instructions there. 
...
::: {.column width="33%" .tiny-text}
![](media/r_logo.png){.centered}

Programming Language
::: 
::: {.column .tiny-text width="33%"}
![](media/rstudio_logo.png){.two-thirds .centered}

IDE (Editor)
:::
::: {.column .tiny-text width="33%"}
![](media/r_markdown_logo.png){.two-thirds .centered}
